...
const ForgotPassword: React.FC<Props> = ({ location, onHideDialog, onShowDialog }) => {
  ...
  useComponentDidMount(() => {
    ...
    if (state?.email) {
      formRef.current.setValues({ email: state.email as string, onlineBusinessId: '' });
    }
  });
  ...
  const handleSubmitForgotPassword = (values: FormValue) => {
    const { email, onlineBusinessId } = values;
    const preferredUsername = `${Tenants.TPA}|${onlineBusinessId}|${email.trim().toLowerCase()}`;
  ...
  const ForgotPasswordSchema = Yup.object().shape({
    ...
    onlineBusinessId: Yup.string().required().businessId(),
  });
                  ...
                  <Grid.Wrap>
                    <Grid.Item variant="is-full">
                      <Input
                        ...
                        errorMessage={touched.onlineBusinessId ? errors.onlineBusinessId : ''}
                        {...getFieldProps('onlineBusinessId')}
                      />
                    </Grid.Item>
                    <Grid.Item variant="is-full">
                      <Input
